...
def create_camera_actor(g, scale=0.02):
    ...
    def animation_callback(vis):
        ...
        with torch.no_grad():
            ...
            for p in droid_visualization.points.items():
                pcd_points += p[1]
            # ?????????????????????? float ?? uchar ????
            pcd_points.points = o3d.utility.Vector3dVector(np.asarray(pcd_points.points).astype(np.float32))
            #pcd_points.colors = o3d.utility.Vector3dVector(np.asarray(pcd_points.colors).astype(np.uint8))
            #pcd_points.colors = o3d.utility.Vector3dVector(np.asarray(pcd_points.colors))
            #   ????????????????????????????????
